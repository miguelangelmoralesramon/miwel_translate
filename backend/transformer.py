...
@Author: miguelangelmoralesramon
"""
...
class SentenceEmbedding(nn.Module):
    ...
    def batch_tokenize(self, batch, start_token, end_token):
        def list_sentence(text):
          ...
          while i < len(text):
              # Check for each special token and append it to the result list if found
              if text[i:i+5] == '<SOE>':
                  result.append('<SOE>')
                  i += 5
              elif text[i:i+5] == '<PAD>':
                  ...
                  # Otherwise, just append the single character
        ...
        def tokenize(sentence, start_token, end_token):
          try:
            sentence_word_indicies = [self.language_to_index[token] for token in list_sentence(sentence)]
